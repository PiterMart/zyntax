...
var AsciiMorph = (function() {
  ...
  function squareOutData(data) {
    ...
    var paddings = {
      x: canvasDimensions.leftAlign ? 0 : Math.floor((canvasDimensions.x - renderDimensions.x) / 2),
      y: Math.floor((canvasDimensions.y - renderDimensions.y) / 2)
    }
    
    // Left Padding (skip when leftAlign)
    for( var i = 0; i < data.length; i++ ) {
      data[i] = repeat(' ', paddings.x) + data[i] + (canvasDimensions.leftAlign ? '' : repeat(' ', paddings.x));
    }
